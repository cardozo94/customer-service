...
//	private ModelMapper mapper = new ModelMapper();
	...
	public boolean createCustomer(@RequestBody CustomerRequest customer) {
		return customerService.createCustomer(converterReq.convert(customer)).blockingGet();
	}

	@PutMapping("/{id}")
	@ResponseStatus(HttpStatus.NO_CONTENT)
	public void updateCustomer(@PathVariable("id") int id, @RequestBody CustomerRequest customer) {
		if (!customerService.updateCustomer(id, converterReq.convert(customer)).blockingGet())
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Customer Not Found");
	}

	@GetMapping
	public List<CustomerResponse> getAllCustomers() {
		return customerService.selectAllCustomers().blockingGet().stream().map(customer -> converterRsp.convert(customer))
				.collect(Collectors.toList());
	}

	@GetMapping("/{id}")
	public CustomerResponse getCustomerById(@PathVariable("id") int id) {
		CustomerResponse customer = converterRsp.convert(customerService.selectCustomerById(id).blockingGet());
		if (customer.getId() == 0)
			throw new ResponseStatusException(HttpStatus.NOT_FOUND, "Customer Not Found");
		return customer;
	}
