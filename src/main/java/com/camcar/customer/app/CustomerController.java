...
//		customer.setId(1);
	...
	public boolean updateCustomer(@PathVariable("id") int id, @RequestBody CustomerRequest customer) {
		return customerService.updateCustomer(id, mapper.map(customer, CustomerServiceDto.class)).blockingGet();
	}
	...
	public CustomerResponse getCustomerById(@PathVariable("id") int id) {
		return mapper.map(customerService.selectCustomerById(id).blockingGet(), CustomerResponse.class);
	}
